...
#aws --profile agsafety ec2  describe-images --image-ids ami-0fdd2abe52514a212 --region us-east-1 | grep "SnapshotId\|Volume\|DeviceName" | awk '{print $4}'
...
#echo "#############################################################################################################################"
#aws ec2 run-instances  --iam-instance-profile Name=CloudWatchrole --image-id ami-28e07e50  --count 1 --instance-type t2.micro --key-name agLSMVJNJ_TRN --security-group-ids sg-0c2db4c5cbb0cb5c2 --vpc-id vpc-1c0ff364 --subnet-id subnet-7002773b  --tag-specifications 'ResourceType=instance,Tags=[{Key=Application,Value=ARISg}]' 'ResourceType=instance,Tags=[{Key=Business Unit,Value=agSafety}]' 'ResourceType=instance,Tags=[{Key=Business_Modle,Value=Single Tenant}]' 'ResourceType=instance,Tags=[{Key=Customer,Value=JNJ}]' 'ResourceType=instance,Tags=[{Key=Environment,Value=DEV}]' 'ResourceType=instance,Tags=[{Key=Name,Value=Test}]' 'ResourceType=instance,Tags=[{Key=Product,Value=Arisg}]' 'ResourceType=instance,Tags=[{Key=Region,Value=Oregon}]' 'ResourceType=instance,Tags=[{Key=Service,Value=EC2}]'--region us-west-2
